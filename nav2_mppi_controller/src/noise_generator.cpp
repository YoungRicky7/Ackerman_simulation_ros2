...
void NoiseGenerator::generateNoisedControls()
{
  ...
  // Generate noises based on student's t-distribution
  // int degrees_of_freedom = 10;

  // xt::xarray<float> standard_normal = xt::random::randn<float>({s.batch_size, s.time_steps});
  // xt::xarray<float> chi_squared = xt::random::randn<float>({s.batch_size, s.time_steps});
  // chi_squared = xt::square(chi_squared);
  // chi_squared = xt::sum(chi_squared, {1});
  // chi_squared = chi_squared / degrees_of_freedom;

  // xt::xarray<float> t_distribution = standard_normal / xt::sqrt(chi_squared);
  // xt::noalias(noises_vx_) = t_distribution * s.sampling_std.vx;
