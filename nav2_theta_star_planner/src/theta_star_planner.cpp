  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  auto planning_time = dur.count() / 1000.0;
  auto iterations = planner_->iterations_;
  auto expansions_log = planner_->nodes_opened;
  auto final_cost = planner_->final_cost_;
  ...
  if (debug)
  {
    ...
    std::cout << "******* " << "path's cost is " << final_cost << " *******" << std::endl;
    std::cout << "******* " << "expansions's size is " << expansions_log << " *******" << std::endl;
    ...
    std::string log_file_path = std::string(home_dir) + "/Theta.txt";
    ...
    if (log_file.is_open())
    {
      if (log_file.tellp() == 0)
      {
        log_file << "expansions planning_time(ms) iterations path_cost" << std::endl;
      }

      log_file << expansions_log << " ";
      log_file << planning_time << " ";
      log_file << iterations << " ";
      log_file << final_cost << std::endl;
      log_file.close();
    }
    else
    {
